...
st.title("⚡🚘EV 사양 데이터셋 기초 탐색")
st.caption("pandas head/tail/shape/info/columns로 데이터셋 기본 정보를 확인합니다.")

# electric_vehicles_spec_2025.csv 가져오기
CSV_PATH = ("..\common\electric_vehicles_spec_2025.csv")

upload_file = st.file_uploader("electric_vehicles_spec_2025.csv 파일을 직접 업로드 할 수 있습니다.(선택사항)", type="csv")
...
if upload_file is not None:
    ...
    try:
        ...
        st.error("❌ EV 사양 파일을 찾을 수가 없습니ㄴ다.")
...
if df is not None:
    ...
    # 단 한 줄로 수치형 데이터뿐 아니라 문자형 데이터의 자료수(count), 고유값 수(unique), 결측치 유무 등을 알아서 표로 만들어 줌.
    # 행/열을 뒤집어(.T) 보면 한눈에 보기 편함.
